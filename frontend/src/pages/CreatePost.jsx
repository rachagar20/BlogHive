...
const CreatePost = () => {
  ...
  const deleteCategory = (i) => {
    ...
    updatedCats.splice(i, 1); // Fix: delete only the selected category
    setCats(updatedCats);
  };

  const addCategory = () => {
    if (cat.trim()) { // Ensure category is not empty
      let updatedCats = [...cats];
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-grow md:px-[200px] mt-8 pr-5 pl-5">
        <div className="pb-5 border border-gray-300 rounded-lg shadow-lg flex flex-col justify-center items-center bg-white">
          ...
          <form className="w-full flex flex-col justify-center items-center space-y-4 md:space-y-8 mt-4" onSubmit={handleCreate}>
            <div className="w-2/3">
            ...
            <div className="w-2/3 flex flex-col space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {cats?.map((c, i) => (
                  <div key={i} className="flex items-center space-x-2 bg-gray-200 px-2 py-1 rounded-md">
                    <p>{c}</p>
                    <button
            ...
            <button
              type="submit"
              className="bg-indigo-500 w-full mx-auto text-white font-semibold px-4 py-2 md:text-xl text-lg rounded-lg hover:bg-indigo-700 transition duration-300"
            >
              Create
            </button>
          </form>
